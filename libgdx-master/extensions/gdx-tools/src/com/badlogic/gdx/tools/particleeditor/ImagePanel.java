...
class ImagePanel extends EditorPanel {
	...
	public ImagePanel (final ParticleEditor editor, String name, String description) {
		...
		JPanel contentPanel = getContentPanel();
		{
			...
			JButton addButton = new JButton("添加图像");
			buttonsPanel.add(addButton);
			addButton.addActionListener(new ActionListener() {
				public void actionPerformed (ActionEvent event) {
					FileDialog dialog = new FileDialog(editor, "打开图片", FileDialog.LOAD);
			...
			JButton defaultButton = new JButton("默认图像");
			...
			JButton defaultPremultButton = new JButton("默认图像 (Premultipled Alpha图像)");
		...
		}
		{
			...
			contentPanel.add(modesPanel, new GridBagConstraints(1, 0, 1, 1, 1, 0, GridBagConstraints.NORTHWEST,
				GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			JLabel label = new JLabel("粒子模式:");
			modesPanel.add(label);
			ButtonGroup checkboxGroup = new ButtonGroup();
			JRadioButton singleCheckbox = new JRadioButton("单一", editor.getEmitter().getSpriteMode() == SpriteMode.single);
			...
			JRadioButton randomCheckbox = new JRadioButton("随机", editor.getEmitter().getSpriteMode() == SpriteMode.random);
			...
			JRadioButton animatedCheckbox = new JRadioButton("序列帧", editor.getEmitter().getSpriteMode() == SpriteMode.animated);
		...
		}
		{
			...
			imagesPanel.add(upButton, new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.NORTHWEST,
				GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
